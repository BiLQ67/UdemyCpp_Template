...
bool is_dead(const Position &player, Obstacles &obstacles)
{
    bool dead = false;
...
unsigned int random_position()
{
    ...
    std::uniform_int_distribution<unsigned int> random_distribution{1, 4};
...
void print_game_state(const GameState &game_state)
{
    ...
    {
        for (unsigned int i2 = 0; i2 < LEN_X; i2++)
        {
            std::cout << game_state[i][i2] << " ";
        }
        std::cout << std::endl;
    }
}
